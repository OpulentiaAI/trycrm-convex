...
Paul Cushman's Climate Week NYC 2026 dataset ported verbatim into the CRM (2026-09-16). Security review run and its findings fixed. Timestamps: review 2026-08-14 06:40 UTC, fixes 2026-08-14 07:05 UTC. DeepSeek and Grok added as provider options 2026-08-14 07:22 UTC.

### Added

- Paul data port: all 31 files from `paul-data/paul-large-datafiles/` committed verbatim, and every dataset parsed into nine new `paul*` tables — 915 merged Climate Week events (official 838 + 77 Luma-only, screening annotations joined by slug), 32 Luma verifications, the 480-name candidate universe, 26 corrected scored identities, 24 lane profiles, 79 entity↔event relationships, 12 audit findings, 24 intent mappings, and 12 context documents with sha256 (`convex/schema.ts`, `convex/paul.ts`, `convex/paulData/` generated by `scripts/build-paul-seed.mjs`) (2026-09-16)
- Seed is now Paul's workspace: Paul Cushman (owner) and Jeremy Alston (member), the 26 scored entities as companies carrying score/disposition/ICP-fit/warm-path custom fields plus evidence-ledger facts and a research note each, and two real agent tasks. No deals — the source data is explicitly not a CRO pipeline (`convex/model/paulSeed.ts` replaces `convex/model/seed.ts`) (2026-09-16)
- Four new app pages: Events (915 events with screening/top-N tabs, search, expandable evidence and Luma verification), Universe (candidate status browser), Research (scored identities, lane profiles, audit findings, intent map), Dossier (rubric, verification summary, document library) (2026-09-16)
- Embedded MCP endpoint: `POST /mcp` serves the app's public functions as MCP tools via `@vibeflowai/convex-mcp` — paul.* reads + idempotent seeds, CRM create/update writes (no destructive removes exposed), stats/analysis queries, demo controls, plus `paul://` resources and a `paul.briefing` prompt carrying the dataset semantics. Open by default for local builds; `MCP_AUTH_TOKEN` env var turns on Bearer auth (`convex/mcp.ts`, mounted in `convex/http.ts`) (2026-09-16)
...
- Theme swapped from the default Mona Sans/Composio palette to the Ramp Ryu design tokens: Lausanne from `assets.ramp.com`, Ryu dark and light palettes mapped onto the existing token names, body weight 300 with 400 headings, square radius (`src/index.css`; `@fontsource-variable/mona-sans` removed) (2026-09-16)
- The demo reset cron now reseeds Paul's canonical data instead of the fictional Comp AI workspace (`convex/demo.ts`) (2026-09-16)
